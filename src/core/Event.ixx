...
export namespace awen::core
{
    ...
    {
        /// @brief Logical key identifier; values match SDL3 keycodes.
    ...
    /// @brief Mouse motion event.  Coordinates are in window/logical space.
    ...
    /// @brief Mouse wheel event.  Positive `dy` scrolls up.
    ...
    /// @brief Window resize event carrying the new logical drawable size.
    ...
    /// @brief Sum type of all input events dispatched through `Engine::onInputEvent`.
    using Event = std::variant<EventKeyboard, EventMouseButton, EventMouseMotion, EventMouseWheel, EventWindowResize>;
}
